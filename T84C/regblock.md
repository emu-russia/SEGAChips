...
|---|---|---|
|clk|ClkGen, по всему чипу | TBD. Используется по большей части в верхней вспомогательной логике. |
|cclk|ClkGen, по всему чипу |Комплемент clk|
|db \[7:0\]| |Внутренняя шина данных|
|ab \[15:0\]| |Шина адреса для падов |
|regs_to_db| |Regs1 -> Databus |
|db_to_regs| |Databus -> Regs1 |
|n_db_to_regs| |Комплемент db_to_regs|
|n_pch_bl| |0: Bit Line precharge для Regs1/Regs2. Подзарядка bitline шлангов перед актуализацией их значения|
|wl \[13:0\]| |Word Line enable для регистра с соответствующим индексом|
|regs1_to_regs2| |Regs1 <-> Regs2. Для соединения используется "любимый всеми" элемент `tran` (двунаправленный элемент соединения двух шин)|
|n_regs1_to_regs2| |Комплемент regs1_to_regs2|
|idu_to_regs2| |IDU -> Regs2|
...
|n_idu_clr| |0: Установить выход IDU в 0 независимо от результатов вычислений (Note: выход IDU в инверсной логике и этот управляющий сигнал тоже в инверсной логике, грёбаные неформалы.. То есть когда этот n_idu_clk=0 - выход равен 1 (инверсное значение 0). Будьте внимательны)|
...
Ячейки регистров представляют собой типовые 6T-Sram Cell. Вместо использования Sense Amp применяется олдскульная техника подзарядки bitline (Precharge).
